use crate::{cpu::CpuState, common::Memory};
...
pub fn parse_instruction(cpu_state: &mut CpuState, prg_slice: &[u8]) -> Result<Instruction, String> {
    // TODO! remove the side effects in the flag logic from this function
    if prg_slice.len() == 0 {
        return Err(format!("Could not parse Opcode, not enough bytes"));
    }
    let (opcode, mode, base_cycles) = decode_opcode(prg_slice[0])?;
    let param = decode_param(cpu_state, &prg_slice[1..], &mode);
    let cycles = base_cycles + compute_extra_cycles(cpu_state, &opcode, &mode);
    Ok(Instruction {
        opcode, param, cycles
    })
}

pub fn decode_param(cpu_state: &mut CpuState, prg_slice: &[u8], mode: &AddressingMode) -> Param {
    ...
    match mode {
        ...
        AddressingMode::Accumulator => {
            Param::Value(cpu_state.reg_a)
        },
        AddressingMode::Immediate | AddressingMode::Relative => {
            Param::Value(cpu_state.read_byte(cpu_state.program_counter))
        },
        AddressingMode::IndirectJump => {
            ...
            let mem_addr = cpu_state.read_two_bytes(cpu_state.program_counter);

            // read the two bytes from memory and form it into a mem addr
            let mem_addr = if mem_addr & 0x0FF == 0x0FF {
                let lsb = cpu_state.read_byte(mem_addr) as u16;
                let msb = cpu_state.read_byte(mem_addr & 0xFF00) as u16;
                (msb << 8) + lsb
            } else {
                cpu_state.read_two_bytes(mem_addr)
            };
        ...
        AddressingMode::ZeroPage => {
            ...
            let zero_page_addr = cpu_state.read_byte(cpu_state.program_counter) as u16;
            Param::Address(zero_page_addr)
        },
        AddressingMode::ZeroPageIndexX => {
            let zero_page_addr = cpu_state.read_byte(cpu_state.program_counter).wrapping_add(cpu_state.reg_x) as u16;
            Param::Address(zero_page_addr)
        },
        AddressingMode::ZeroPageIndexY => {
            let zero_page_addr = cpu_state.read_byte(cpu_state.program_counter).wrapping_add(cpu_state.reg_y) as u16;
            Param::Address(zero_page_addr)
        },
        AddressingMode::AbsoluteIndexX => {
            // Form <instruction> <addr>, X where <addr> is u16, specifies the value of read(<addr> + 1)
            let orig_addr = cpu_state.read_two_bytes(cpu_state.program_counter);
            let orig_msb = (orig_addr >> 8) as u8;
            let mem_addr = orig_addr.wrapping_add(cpu_state.reg_x as u16);
            let msb = (mem_addr >> 8) as u8;
            cpu_state.page_cross_flag = (orig_msb != msb);
            Param::Address(mem_addr)
        },
        AddressingMode::AbsoluteIndexY => {
            ...
            let orig_addr = cpu_state.read_two_bytes(cpu_state.program_counter);
            let orig_msb = (orig_addr >> 8) as u8;
            let mem_addr = orig_addr.wrapping_add(cpu_state.reg_y as u16);
            let msb = (mem_addr >> 8) as u8;
            cpu_state.page_cross_flag = (orig_msb != msb);
            Param::Address(mem_addr)
        },
        AddressingMode::IndirectX => {
            ...
            let mem_addr = cpu_state.read_two_page_bytes(zero_page_addr);
            Param::Address(mem_addr)
        },
        AddressingMode::IndirectY => {
            let zero_page_addr = cpu_state.read_byte(cpu_state.program_counter) as u16;
            // TODO: may need to re-evaluate how this is done when there's a page cross
            let orig_addr = cpu_state.read_two_page_bytes(zero_page_addr);
            let orig_msb = (orig_addr >> 8) as u8;
            let mem_addr = orig_addr.wrapping_add(cpu_state.reg_y as u16);
            let msb = (mem_addr >> 8) as u8;
            cpu_state.page_cross_flag = (orig_msb != msb);
